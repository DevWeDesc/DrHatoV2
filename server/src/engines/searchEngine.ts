...
export async function searchEngine({
  ...
  switch (true) {
    ...
    case !!customerName:
      const customerResults = await prisma.pets.findMany({
    ...
    case !!petName:
      const petResults = await prisma.pets.findMany({
    ...
    case !!codPet:
      const codPetResulst = await prisma.pets.findFirst({
    ...
    case !!typePaymentClient:
      const IstypePaymentClientResult = await prisma.queuesForPet.findMany({
        where: { queueIsDone: true },
        include: {
          medicine: {
            include: {

...
export const userController = {
  createUser: async (request: FastifyRequest, reply: FastifyReply) => {
    const { email, username, password, userType, userIsVet, crmv, role, name } =
    ...
    await contract.userHasAllToBeCreated(
      { email, password, username },
      "Usuário não tem todos campos obrigatórios"
    );
  ...
  loginUser: async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    try {
      ...
      const checkedPassword = await contract.checkPassword(
        { username, password },
        "Senha incorreta"
      );
  ...
  editUser: async (request: FastifyRequest, reply: FastifyReply) => {
    const { id }: any = request.params;
    const { email, username, password, userIsVet, crmv, role } =
      UserSchema.parse(request.body);
    const hashedPassword = await bcrypt.hash(password, 10);
    try {
  ...
  searchVetByName: async (
    ...
    try {
      ...
      const vets = await prisma.user.findMany({
        where: {
          consultName: { contains: consultName, mode: "insensitive" },
          role: "VETERINARIAN",
        },
      });
